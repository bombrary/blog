...
実は`Circle`を直接持たせる方法でもやってみたが、実装が少し面倒だった。`Model`に`Circles`を持たせた場合、ドラッグ中の`Circle`と`Circles`に含まれている`Circle`とのデータの同期を取る必要がある。同期を取りたくなければ、`Circles`からドラッグ中の`Circle`を一旦削除して、ドラッグ終了後に再び戻せばよい。

そんなことやるよりも、最初から`Model`にはドラッグ中の`Id`を持たせておいて、それを使って`Circles`内の`Circle`を操作すれば良さそう。ということで、今回は`Id`のみ持たせる方針とする。

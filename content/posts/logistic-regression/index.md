---
title: "ロジスティック回帰 メモ"
date: 2021-06-27T20:51:01+09:00
draft: true
...
このような $f$ は作りたいモデルによって適当に決める必要がある。
...
つまり、線形分離可能なデータに対しては、今回の $\sigma(\bm{w}^T \bm{x})$ を使ったモデルが利用できる。
...
$N$ 個のデータが実現されたということは、それらが実現される確率は高いと考えるのが適当である。
...
これを微分すれば良いのだが、ここで、シグモイド関数の微分についていくつか補足する。
...
コードに投げ易くするために、ベクトルと行列を使った表現に変形する。
